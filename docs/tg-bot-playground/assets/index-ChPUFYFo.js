...
const makeRunnableBot = (tsTextModel, worker) => (state) => tsTextModel.getJsCode().then((code) => {
  ...
  console.warn("cannot run bot", error2);
});
...
const fetchText = (path) => fetch(path).then((_) => _.text());
const getMonacoLoader = () => {
...
const makeTsTextModel = async (monaco) => {
  ...
  return {
    ...
    getJsCode: async () => {
      ...
      return {
        defaultExport,
        serialized: serialize(defaultExport == null ? void 0 : defaultExport.default)
      };
    }
  };
};
async function getDefaultExport(code) {
  try {
    ...
    const module = await __vitePreload(() => import(
      /* @vite-ignore */
      `data:text/javascript,${encodedCode}`
...
async function setup() {
  const state = makeGlobalState(module_default);
  module_default.store("state", state);
  ...
  botLauncher.worker.onmessage = (event) => {
    ...
    console.log("got message from worker", data2);
    if (!data2) return;
    if (data2.botState) {
      Object.assign(state.bot, data2.botState);
    }
    state.botUpdates.push(data2);
  };
}
